...
def compare_similarity(paras, words, flag=0):
    ...
    paras_counter = Counter(paras_chinese)
    # 计算正确词汇和错误词汇的个数
    d_word = {}
    count = 0
    wrong = 0
    ...
    for word in words_chinese:
        if word in paras_chinese:
            if d_word.get(word, 0) < paras_counter[word]:
...
def make_window_jiyi(content="你要记住我"):
    layout = [
        ...
        [sg.Button("已记住"), sg.Button("换一句"), sg.Button("自定义")],
        [sg.Multiline(size=(80, None), key="-customized-", visible=False)]
    ]
        ...
        if window == window_menu and event in (sg.WIN_CLOSED, '退出程序'):
            ...
            if event == '已记住':
                ...
                if event == "确定":
                    ...
                    window_jiyi.close()
                    window_jiyi = make_window_jiyi(paras)
                # texts = normal_cut_sentence(values["-customized-"])
